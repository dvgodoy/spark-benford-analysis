...
package object util {
  ...
  def addMoments(x: Moments, y: Moments) = Moments(x.n+y.n, x.m1+y.m1, x.m2+y.m2, x.m3+y.m3, x.m4+y.m4, x.prod+y.prod)

  case class Stats(n: Double, mean: Double, variance: Double, skewness: Double, kurtosis: Double, pearson: Double)
  def calcMean(m1: Double) = m1
  ...
  private case class AliasTable(mobProb: DenseVector[Double], aliases: DenseVector[Int], nOutcomes: Int)
  private def buildAliasTable(prob: DenseVector[Double]): AliasTable = {
    ...
    val mobProb = DenseVector(prob.map { param => param / sum * nOutcomes }.toArray)
    val (iSmaller, iLarger) = (0 until nOutcomes).partition(mobProb(_) < 1d)
    ...
    while (smaller.nonEmpty && larger.nonEmpty) {
      ...
      aliases(small) = large
      mobProb(large) -= (1d - mobProb(small))
      if (mobProb(large) < 1)
        smaller.push(large)
      else
        larger.push(large)
    ...
    AliasTable(mobProb, aliases, nOutcomes)
  }

  private def findOutcome(aliasTable: AliasTable, rollToss: (Int, Double)): Int = if (rollToss._2 < aliasTable.mobProb(rollToss._1)) rollToss + 1 else aliasTable.aliases(rollToss) + 1
